...
const login = async (ctx, next) => {
  ...
  try {
    ...
    const [results] = await ctx.state.db.query(
    ...
    ctx.body = { user, token, refresh_token };
  } catch (err) {
    console.error(err);
...
const register = async (ctx, next) => {
  ...
  try {
    ...
    const insertedId = await createUser(ctx.state.db, username, password);
    if(!insertedId) throw new Error("Username already exists");

    const [token, refresh_token] = createTokens({ id: insertedId });
    ctx.status = 201;
    ctx.body = { id: insertedId, token, refresh_token };
  } catch (err) {
    console.error(err);
    ctx.body = { error: err.message };
  }
